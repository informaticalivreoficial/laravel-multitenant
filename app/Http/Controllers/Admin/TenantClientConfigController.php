...
class TenantClientConfigController extends Controller
{
    ...
    public function update(ConfiguracoesRequest $request, $config)
    {
        ...
        if(!empty($request->file('metaimg'))){
            $tenant->metaimg = $request->file('metaimg')->storeAs('configuracoes/'.$tenant->uuid, 'metaimg-'.Str::slug($request->nomedosite)  . '.' . $request->file('metaimg')->extension());
        }
        
        if(!empty($request->file('logomarca'))){
            $tenant->logomarca = $request->file('logomarca')->storeAs('configuracoes/'.$tenant->uuid, 'logomarca-'.Str::slug($request->nomedosite)  . '.' . $request->file('logomarca')->extension());
        }
        
        if(!empty($request->file('logomarca_admin'))){
            $tenant->logomarca_admin = $request->file('logomarca_admin')->storeAs('configuracoes/'.$tenant->uuid, 'logomarca-admin-'.Str::slug($request->nomedosite)  . '.' . $request->file('logomarca_admin')->extension());
        }
        
        if(!empty($request->file('favicon'))){
            $tenant->favicon = $request->file('favicon')->storeAs('configuracoes/'.$tenant->uuid, 'favivon-'.Str::slug($request->nomedosite)  . '.' . $request->file('favicon')->extension());
        }
        
        if(!empty($request->file('marcadagua'))){
            $tenant->marcadagua = $request->file('marcadagua')->storeAs('configuracoes/'.$tenant->uuid, 'marcadagua-'.Str::slug($request->nomedosite)  . '.' . $request->file('marcadagua')->extension());
        }
        
        if(!empty($request->file('imgheader'))){
            $tenant->imgheader = $request->file('imgheader')->storeAs('configuracoes/'.$tenant->uuid, 'imgheader-'.Str::slug($request->nomedosite)  . '.' . $request->file('imgheader')->extension());
        }
