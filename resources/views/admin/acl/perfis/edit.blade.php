...
<div class="row mb-2">
    ...
    <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item"><a href="{{route('perfis')}}">Perfis</a></li>
        ...
        <div class="row">
            <div class="col-12">
                <div class="card card-teal card-outline card-outline-tabs">
                    ...
                    <div class="card-body">
                        <form action="{{ route('perfis.update', ['id' => $perfil->id]) }}" method="post" enctype="multipart/form-data" autocomplete="off">
                        @csrf
                        @method('PUT')  
                            ...
                            <div class="tab-pane fade show active" id="custom-tabs-conteudo" role="tabpanel" aria-labelledby="custom-tabs-conteudo-tab">
                                ...
                                <div class="row">
                                    <div class="col-6">
                                        <div class="form-group">
                                            ...
                                            <input class="form-control" name="name" placeholder="Título" value="{{old('name') ?? $perfil->name}}">
                                        </div>
                                    </div>                                    
                                    <div class="col-3">
                                        <div class="form-group">
                                            ...
                                            <select name="status" class="form-control">
                                                <option value="1" {{ (old('status') == '1' ? 'selected' : ($perfil->status == '1' ? 'selected' : '')) }}>Publicado</option>
                                                <option value="0" {{ (old('status') == '0' ? 'selected' : ($perfil->status == '0' ? 'selected' : '')) }}>Rascunho</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>                                
                                <div class="row">                                    
                                    <div class="col-12">   
                                        ...
                                        <x-adminlte-text-editor name="content" v placeholder="Conteúdo do post..." :config="$config">{{ old('content') ?? $perfil->content }}</x-adminlte-text-editor>                                                      
                                    </div>
                                </div> 
                            </div> 
                        </div> 
